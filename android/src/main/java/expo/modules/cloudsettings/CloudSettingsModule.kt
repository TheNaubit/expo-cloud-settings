...
class CloudSettingsModule : Module() {
  ...
  private fun currentAccountId(): String? {
    ...
    return account.id ?: account.email ?: account.account?.name
  }
  ...
  private fun request(
    ...
    try {
      ...
      val bodyText = if (stream != null) {
        BufferedReader(InputStreamReader(stream, StandardCharsets.UTF_8)).use { reader -> reader.readText() }
      } else {
        ""
      }
  ...
  private fun uploadSnapshot(token: String) {
    ...
    if (file != null) {
      ...
      if (response.code in 200..299) {
        ...
        currentAccountId()?.let { metaEditor.putString(KEY_ACCOUNT_ID, it) }
  ...
  private fun syncFromRemote(force: Boolean = false) {
    ...
    appContext.backgroundCoroutineScope.launch {
      try {
        ...
        val cachedAccountId = meta.getString(KEY_ACCOUNT_ID, null)
        ...
        accountId?.let { metaEditor.putString(KEY_ACCOUNT_ID, it) }
        metaEditor.apply()
        hasSyncedOnce = true
      } finally {
        isSyncing.set(false)
